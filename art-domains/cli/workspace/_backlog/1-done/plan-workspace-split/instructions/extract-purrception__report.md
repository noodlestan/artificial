...
All goals landed. The `noodlestan/purrception` repo is a standalone entity extraction monorepo: root scaffold + migrated tree (4 packages), builds/lints/passes CI on its own with the toolchain fully declared, the build bins rewired to `@noodlestan/esbuild`, and zero cross-family dependencies. The workspace records list the new repo. The monorepo and the migration source are untouched.
...
`purrpose → @no-comply/solid-primitives` reverse edge: not applicable at this extraction (purrception has no reverse edges).
