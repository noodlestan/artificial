...
### MD Art Roundtrip (WORKING)

- `_backlog/3-now/milestone-md-art-roundtrip/milestone.md` — — Migrate poc-parse into proper packages (`primitives`, `parser`), verify lossless roundtrip (parse `.art` → serialize back → zero diffs), archive poc-parse, and publish v0.0.1. Each commit guarantees no source change breaks the roundtrip contract. Test harnesses: fixtures in `spec/`, automation to detect diffs in `.art.json` files.
