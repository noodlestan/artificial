...
- **POC Step 2 — Core record schema (done, commit fd67848):** author composable TS types in `cli/poc-parse` per ecosystem best-practices (`_research.md`) — discriminated union on `construct`, open per-position registries with derived unions (mdast `RootContentMap` pattern), position metadata, derive depth/parent from tree. Schema-first: TS is the metalanguage ground truth; the `.art` spec is the object language, reconciled later by the bootstrap check.
- **POC Step 3 — Smoke-parse the corpus through micromark (next):** 6 ADR files + one spec file; inspect the token event stream; confirm `**Field:**` (incl. block fields), `::READ`, `#tags`, code fences tokenize as expected.
