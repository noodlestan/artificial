...
**commit.Id:** `smoke-parse-section-block`
...
The working directory is the workspace (meta-repo, orchestrates cross-repo work). These instructions are scoped to changes in `repos/{repo}`, where verification steps, commits, and pushes are made, and to artefacts in the workspace itself, such as feedback reports or backlog items. File paths in this instruction are workspace-relative unless explicitly noted otherwise.
...
3. **User interaction is minimal.** The user relays this instruction to the delegator and expects a light confirmation: a happy face and up to 3 bullet points — done `smoke-parse-section-block`, created `src/parse/smoke-parse.ts`, thumbs up. If something goes horribly wrong, report the blocker instead of a summary.

## Goals

Prove that micromark can tokenize the art syntax before building the full record builder. Parse the smoke-parse target files (6 ADR files + `section-block.art`) through micromark and inspect the token event stream to confirm:
...
- `repos/artificial/_backlog/plan-poc-parse/plan.md` — the plan; this commit is `smoke-parse-section-block`.
- `repos/artificial/_guide.md` — the Artificials System overview: the compiler pipeline, the declaration model (`# Kind: Name`), and the kind-agnostic parser. Context only.
- `repos/artificial/_architect.md` — Approach (micromark substrate) + Step 3 (smoke-parse the target files).
- `repos/artificial/_wip.md` — only to identify the current step; NEVER modify it.
- `repos/artificial/architecture/records/adr/_research.md` — best practices 1–6; esp. #1 (two layers) and #6 (early spike insight: `# Kind: Name` is plain ATX heading).
- `repos/artificial/art-js/spec/grammar/constructs/structural/section-block.art` — the spec file to include in the smoke-parse target.
...
Create `repos/artificial/art-js/cli/poc-parse/src/parse/smoke-parse.ts` that:

1. Reads the smoke-parse target files:
...
5. Writes findings to `repos/artificial/_backlog/plan-poc-parse/instructions/smoke-parse-section-block__findings.md` with:
...
- Only create: `repos/artificial/art-js/cli/poc-parse/src/parse/smoke-parse.ts` and `repos/artificial/_backlog/plan-poc-parse/instructions/smoke-parse-section-block__findings.md`. Do NOT touch `src/index.ts`, `src/parse/types.ts`, `package.json`, tsconfigs, or any other existing file.
...
The artificials POC step 3 goal is met: the target files parse through micromark, token events are inspected, and findings are recorded in a report that settles the substrate decision.

**Verification steps**

- Execute `npx tsc --noEmit` in `repos/artificial/art-js/cli/poc-parse/` — resolves the repo root's typescript and `@types/node` (upward node_modules walk); must exit 0 with no errors. If `npx` attempts to download typescript, run the repo-root binary directly: `../../../../node_modules/.bin/tsc --noEmit`.
- Execute `npx tsx src/parse/smoke-parse.ts` in `repos/artificial/art-js/cli/poc-parse/` — must run without errors and produce output. If `npx` attempts to download tsx, install it first: `npm install -D tsx` (this is allowed for this step only).
- Re-read `repos/artificial/_backlog/plan-poc-parse/instructions/smoke-parse-section-block__findings.md` and confirm it contains:
  - Summary of what tokenizes as expected
  - Surprises or gaps
  - Recommendation: micromark direct vs indirections
- Confirm `git status` shows exactly two new files: `repos/artificial/art-js/cli/poc-parse/src/parse/smoke-parse.ts` and `repos/artificial/_backlog/plan-poc-parse/instructions/smoke-parse-section-block__findings.md`.
...
4. Render the report file to `repos/artificial/_backlog/plan-poc-parse/instructions/smoke-parse-section-block__report.md` — reports are co-located with their instruction file (`plan-{id}/instructions/{id}__report.md`, per `files/index.md`).
5. Generate the response and send it back to the delegator.
6. Keep the response terse per the Working Agreements: happy face + up to 3 bullet points (done `smoke-parse-section-block`, created `src/parse/smoke-parse.ts`, thumbs up). The full trail lives in the report file; never repeat it in chat.
